...
INSERT INTO book VALUES(2004, cart'200', '아무튼,외국어', 22000, 50, '조지영', '위고', '2019-04-20', 'nothing.jpg', '모든 나라에는 철수와 영희가 있다', 10, '2019-04-28');
...
# 문제1] member테이블에서 id가 qwer인 회원의 이름과 비밀번호 가져오기
select member_name, member_pw from member where member_id='qwer';
# 문제2] book 테이블에서 book_kind가 문학(100)인 책 정보 가져오기
select*from book where book_kind=100;
# 문제3] book 테이블에서 가격이 15,000원 이하인 책 정보 가져오기
select*from book where book_price <= 15000;
# 문제4] book 테이블에서 문학(100)이 아닌 책 정보만 가져오기
select*from book where book_kind !=100;
# 문제5] book테이블에서 2020년 이전에 등록된 책정보만 가져오기
select*from book where book_reg_date < '2020-01-01';
# 문제6] book테이블에서 컴퓨터(300)이면서 가격이 15,000원 이상인 책정보 가져오기
